...
const NavLinkButtons = ({ title, href, dropdown }: NavLinkButtonsProps) => {
  ...
  return (
    <div
      className="relative z-0 hover:z-20 capitalize transition-all duration-300 hover:text-dark-color cursor-pointer group"
    ...
    >
      {!hasDropdown ? (
        ...
        <div className="flex items-center gap-1">
          ...
          {open && (
            <div className="absolute top-7 left-0 w-52 text-black bg-white rounded-md shadow-lg z-10 overflow-hidden">
              {dropdown.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
